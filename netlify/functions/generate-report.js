...
exports.handler = async (event) => {
    ...
    try {
        ...
        ${gasType === 'Venous' ? "CRITICAL VENOUS SAMPLE INSTRUCTION: Generate a 'po2' value that is low (typically 4-6 kPa) and a 'pco2' value that is slightly elevated compared to arterial norms to reflect that this is a venous sample." : ""}
        ...
        // --- UPDATED SECTION: CONDITIONAL REFERENCE RANGES ---
        const phRef = gasType === 'Venous' ? '(7.310 - 7.410)' : '(7.350 - 7.450)';
        const pco2Ref = gasType === 'Venous' ? '(5.30 - 6.70)' : '(4.67 - 6.00)';
        const po2Ref = gasType === 'Venous' ? '(4.00 - 6.70)' : '(10.67 - 13.33)';

        formattedReport += formatLine('pH', reportData.ph, '', phRef);
        formattedReport += formatLine('PCO₂', reportData.pco2, 'kPa', pco2Ref);
        formattedReport += formatLine('PO₂', reportData.po2, 'kPa', po2Ref);
